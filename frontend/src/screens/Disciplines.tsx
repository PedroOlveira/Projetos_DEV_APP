...
type Item = { id:number; name:string; workload?:number; professor_id?:number };
export default function Disciplines() {
  const [items, setItems] = useState<Item[]>([]);
  const [form, setForm] = useState<any>({});
  const load = async () => { const { data } = await api.get("/disciplines"); setItems(data); };
  useEffect(() => { load(); }, []);
  const save = async () => { try { await api.post("/disciplines", form); setForm({}); load(); } catch (e) { Alert.alert("Erro","Não foi possível salvar"); } };
