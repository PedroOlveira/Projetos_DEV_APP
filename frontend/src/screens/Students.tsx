...
type Item = { id:number; name:string; code:string; course?:string; email?:string };
export default function Students() {
  const [items, setItems] = useState<Item[]>([]);
  const [form, setForm] = useState<any>({});
  const load = async () => { const { data } = await api.get("/students"); setItems(data); };
  useEffect(() => { load(); }, []);
  const save = async () => { try { await api.post("/students", form); setForm({}); load(); } catch (e) { Alert.alert("Erro","Não foi possível salvar"); } };
